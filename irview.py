...
class ImageWindow:
    def __init__(self, L, fh5=None, rate=1.0e-8):
        try:
            # Assume sample rate is const.
            self.rate = 1.0 / L.metadata.sample_rate[0]
        except:
            traceback.print_exc()
        ...
        key_press_cids = self.fig1.canvas.callbacks.callbacks.get(
            "key_press_event", {}
        ).copy()
        ...
        self.cid_key = self.fig1.canvas.mpl_connect("key_press_event", self._onkeypress)
    ...
    def EndFeature(self):
        self.features[self.fid] = [
            self.active_feature_name,
    ...
    def ShowRadargram(self, cmap="gray", c=1.68e8, repaint=False, lum_scale=None):
        ...
        if len(self.ax1.images) == 0 or repaint is True:
            self.ax1.cla()
            self.ax1.imshow(
                self.arr, aspect="auto", cmap=cmap, vmin=-lum_bound, vmax=lum_bound
            )
            locs = self.ax1.get_yticks()
            self.ax1.set_yticklabels(locs * self.rate * 1e9)

        # Draw nodes
        drawxy = lambda xy: self.ax1.plot(
        ...
        if len(self.features) > 0:
            ...
            drawline = lambda lsxy: self.ax1.plot(
                [i[0] for i in lsxy[1]], [i[1] for i in lsxy[1]], "--r"
            )
            lines_ = map(drawline, self.features.values())

            labelfeature = lambda key, lsxy: self.ax1.text(
                lsxy[1][-1][0], lsxy[1][-1][1] - 20, str(key), fontsize=12, color="r"
            )
    ...
    def GetDigitizerFilename(self):
        fnm = (
            "englacial/"
            + os.path.basename(self.datafile).split(".")[0]
            + "_line"
            + str(self.line)
            + ".txt"
        )
        ...
        for fdict in dict_list:
            ...
            for key in keys:
                ...
                if isinstance(vals, __builtins__.tuple):
                    fout.write(
                        "{0}\t{1}\t{2}\t{3}\n".format(key, vals[0], vals[1], vals[2])
                    )
            fout.write("\n")
    return
...
def HandleCommand(s, S, IW, L):
    ...
    if args[0] in ("exit", "q", "quit"):  # EXIT
        ...
        try:
            if "line_{0}".format(
                line
            ) in S.GetLines() and datacapture < S.GetChannelsInLine(int(line)):
                ...
                print(
                    "Line {0} channel {1} does " "not exist".format(line, datacapture)
                )
        except:
            traceback.print_exc()
