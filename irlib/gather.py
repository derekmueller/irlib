...
class Gather(object):
    ...
    def __init__(self, arr, infile=None, line=None, metadata=None, retain=None, dc=0):
        ...
        if self.metadata is not None:
            self.rate = 1.0 / self.metadata.sample_rate[0]
    ...
    def __repr__(self):
        return "Gather instance from {fnm} at line {line}, " "channel {chan}".format(
            fnm=os.path.split(self.infile)[1], line=self.line, chan=self.datacapture
        )
    ...
    def _getkernel(self, width, kind):
        ...
        if kind == "boxcar":
            kernel = 1.0 / width * np.ones((width,))
            ...
            kernel = math.e ** (
                -0.5 * ((n - (width - 1) / 2.0) / (sigma * (width - 1) / 2.0)) ** 2
            )
            kernel /= kernel.sum()
    ...
    def GetDigitizerFilename(self):
        ...
        fnm = (
            "englacial/"
            + os.path.basename(self.infile).split(".")[0]
            + "_line"
            + str(self.line)
            + ".txt"
        )
        ...
        if topofnm is None:
            ...
            try:
                self.topography = np.array(
                    map(
                        lambda x, y: G.sample(x, y)[0],
                        self.metadata.eastings,
    ...
    def RemoveGPSNaNs(self):
        ...
        kill_list = [
            ...
            if (
                np.isnan(self.metadata.eastings[i])
    ...
    def SmoothenGPS(self, win=5):
        ...
        eastings = np.convolve(
            np.hstack(
                [
                    np.ones(win // 2) * eastings0[0],
                    eastings0,
                    np.ones(win // 2) * eastings0[-1],
                ]
            ),
            np.ones(win) / float(win),
            mode="valid",
        )
        northings = np.convolve(
            np.hstack(
                [
                    np.ones(win // 2) * northings0[0],
                    northings0,
    ...
    def DoTimeGainControl(self, ncoef=1.0, npow=1.0, nexp=0.0, gamma=1.0, bias=0.0):
        ...
        for i in range(self.data.shape[1]):
            if self.data[:, i].mean() != 0:
                ...
                self.data[:, i] = (
                    B / np.sqrt(np.mean(B**2)) * np.sqrt(np.mean(self.data[:, i] ** 2))
                )

        self.history.append(("time_gain_control", ncoef, npow, nexp, gamma, bias, dt))
        return

    def DoMurrayGainControl(self, npow=2.0, tswitch=100.0):
        ...
        try:
            ...
            for i in range(self.data.shape[1]):
                ...
                if sum(self.data[:, i] ** 2) != 0:
                    ...
                    # Scale to preserve average amplitude
                    agctrace[np.isnan(agctrace)] = 0.0
                    agctrace = (
                        agctrace
                        / np.sqrt(np.mean(agctrace**2))
    ...
    def DoWindowedSinc(self, cutoff, bandwidth=2.0e6, mode="lowpass"):
        ...
        bw = bandwidth / r
        if max((fc, bw)) > 0.5:
            raise LineGatherError("cutoff or bandwidth exceed Nyquist " "frequency")
        M = int(4 // bw)
        if M % 2 == 1:
            M += 1
    ...
    def DoWienerFilter(self, window=5, noise=None):
        ...
        assert window % 2 == 1
        for i in range(self.data.shape[1]):
            self.data[:, i] = sig.wiener(self.data[:, i], mysize=window, noise=noise)
    ...
    def WaveletTransform(self, trno, m=0):
        ...
        wva, scales, periods, coi = pywavelet.pywavelet(
            y, dt, mother, param, s0, dj, jtot, npad
        )
    ...
    def RemoveTraces(self, kill_list):
        ...
        kill_list.reverse()
        keep_list = list(
            set(range(len(self.metadata.locations))).difference(set(kill_list))
        )
        keep_list.sort()
    ...
    def Dump(self, fnm=None):
        ...
        if os.path.isdir(os.path.split(fnm)[0]):
            ...
            sys.stderr.write(
                "Cache directory does not exist ({0}/)\n".format(os.path.split(fnm)[0])
            )
        return
...
class CommonOffsetGather(Gather):
    ...
    def GetTopoCorrectedData(self):
        ...
        for itr, trace in enumerate(padded_data.T):
            shift = (
                int(round((max_topo - self.topography[itr]) / 1.68e8 / self.rate)) * 2
            )
    ...
    def FindLineBreaks(self, threshold=0.35):
        ...
        try:
            ...
            i = 20
            while i < len(dxnf) - 1:
                u = np.array([dxnf[breaks[-1] : i].mean(), dynf[breaks[-1] : i].mean()])
    ...
    def LineProject_Nearest(self, bounds=None, eastings=None, northings=None, dp=4.0):
        ...
        X, Y, p = self.LineProjectXY(
            bounds=bounds, eastings=eastings, northings=northings
        )
    ...
    def RemoveBadLocations(self, bbox=None):
        ...
        for i, (x, y) in enumerate(
            zip(self.metadata.eastings, self.metadata.northings)
        ):
    ...
    def MigrateFK(self, dx=4.0, t0_adjust=0, verbose=True):
        ...
        for bounds in migsections:
            ...
            X, Y, p = self.LineProjectXY(bounds=bounds)
            if t0_adjust > 0:
                arr = np.roll(self.data[:, bounds[0] : bounds[1]], -t0_adjust, axis=0)
            else:
...
class PickableGather(Gather):
    ...
    def __init__(self, data, infile=None, line=None, metadata=None, retain=None, dc=0):
        ...
        if isinstance(data, Gather):
            super(PickableGather, self).__init__(
                data.data,
                infile=data.infile,
                line=data.line,
            ...
            super(PickableGather, self).__init__(
                data, infile=infile, line=line, metadata=metadata, retain=retain, dc=dc
            )
        ...
        try:
            picks = np.array(
                list(
                    map(
                        first_break_bed,
                        self.data[sbracket[0] : sbracket[1], istart:iend].T,
                    )
                )
            )
    ...
    def PickDC(self, sbracket=(20, 50), bounds=(None, None)):
        ...
        try:
            picks = map(
                first_break_dc, self.data[sbracket[0] : sbracket[1], istart:iend].T
            )
    ...
    def CalcAveragePicks(self, key, picks):
        ...
        try:
            ...
            sys.stderr.write(
                "key and pick arguments to CalcAveragePicks() are" " inconsistent\n"
            )
            avg_picks = None
    ...
    def RemoveMetadata(self, kill_list, update_registers=True):
        ...
        try:
            ...
            self.dc_picks = np.hstack([self.dc_picks[i] for i in keep_list])
            self.dc_phase = np.hstack([self.dc_phase[i] for i in keep_list])
            super(PickableGather, self).RemoveMetadata(
                kill_list, update_registers=update_registers
            )
        except IndexError:
            print("Inconsistent data lengths in {0}".format(repr(self)))
            traceback.print_exc()
