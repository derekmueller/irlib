...
def LoadCoords(line, filename, L):
    ...
    with h5py.File(filename, "r") as f:
        ...
        for name in allnames:
            if isinstance(f[name], h5py.Dataset) and name.split("/")[
                0
            ] == "line_" + str(line):
                paths.append(name)
...
def ExtractAttrs(h5file, outfile=None, fout=None, eastern_hemisphere=False):
    ...
    try:
        ...
        datasets = []
        for name in names:
            if (type(f[name]) == h5py.Dataset) and ("picked" not in f[name].name):
...
def ExtractLine(h5file, line, bounds=(None, None)):
    ...
    with h5py.File(h5file, "r") as f:
        ...
        try:
            ...
            traceback.print_exc()
            sys.stderr.write(
                "Error sorting datasets by location " + "number in ExtractLine()\n"
            )
        ...
        try:
            ...
            sys.stderr.write(
                "bounds kwarg in ExtractLine() must be" + " a two element list or tuple"
            )
        ...
        for dataset in datasets[1:]:
            ...
            if newrow.shape[0] < arr.shape[0]:
                newrow = np.vstack(
                    (newrow, np.zeros((arr.shape[0] - newrow.shape[0], 1)))
                )
            elif newrow.shape[0] > arr.shape[0]:
                arr = np.vstack(
                    (arr, np.zeros((newrow.shape[0] - arr.shape[0], arr.shape[1])))
                )
...
def PlotLine(arr, outfile=None, cmap="gray", title="Radar Line", rate=1e-8, c=1.68e8):
    ...
    plt.yticks(locs, locs * 1.0e9 * rate)
