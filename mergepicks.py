...
def get_pick_fnm(h5, lineno, dirpicking):
    ...
    fnm = os.path.join(
        dirpicking, "{0}_line{1}.csv".format(os.path.basename(h5).split(".")[0], lineno)
    )
    return fnm
...
parser.add_argument(
    "-n",
    "--newpick_priority",
    help="will priviledge new picks over old picks in case of conflict",
    action="store_true",
)
parser.add_argument(
...
for line in linenos:
    ...
    p_updated = pd.DataFrame(
        list(zip(p.fids, list(p.dc_picks), list(p.bed_picks))),
        columns=["FID", "dc", "bed"],
    )

    print(
        " Current survey version has {} traces, with {} dc picks and {} bed picks".format(
    ...
    try:
        ...
        print(
            " Older survey version has {} traces, with {} dc picks and {} bed picks".format(
        ...
        if args.newpick_priority:
            ...
            p_merge.bed_x[p_merge.bed_x.issnull()] = p_merge.bed_y[
                p_merge.bed_x.isnull()
            ]
        else:
            # only take old picks
        ...
        print(
            " Merged survey version has {} traces, with {} dc picks and {} bed picks".format(
        ...
        save_picks(p, line, get_pick_fnm(h5, line, args.outdir))
        print(
            " Could not find/read file {}. New [unmerged] picking file saved".format(
                get_pick_fnm(oldh5, line, oldpickdir)
            )
        )
