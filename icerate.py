...
class RatingWindow(object):
    def __init__(self, L, picks, ratings=None, rate=1.0e-8, bias=0.02):
        self.isopen = True
        self.bias = bias
        self.rate = rate
        self.picks = picks
        ...
        if ratings is None:
            ...
            self.ratings = -9 * np.ones((len(picks)))
            sys.stderr.write(
                "ratings passed to RatingWindow have inconsistent length\n"
            )
    ...
    def ShowRadargram(self, cmap="gray", rate=1e-8, c=1.68e8, repaint=False):
        ...
        self.ax1.plot(
            (self.cur_trace + self.interval, self.cur_trace + self.interval),
            (0, self.arr.shape[0]),
            "-y",
            alpha=0.6,
            linewidth=2.0,
        ...
        try:
            ...
            upper_line = map(
                lambda n: (n == 1004 and np.nan or n),
                [i + 5 for i in self.picks[xa:xb]],
            )
            lower_line = map(
                lambda n: (n == 994 and np.nan or n), [i - 5 for i in self.picks[xa:xb]]
            )
            self.ax1.plot(
                list(range(xa, xb)), list(upper_line), "-y", alpha=0.6, linewidth=2.0
            )
            self.ax1.plot(
                list(range(xa, xb)), list(lower_line), "-y", alpha=0.6, linewidth=2.0
            )
        except:
            traceback.print_exc()
...
def get_pickfnm(infile, line):
    return os.path.join(
        "picking", os.path.basename(infile).split(".")[0] + "_line" + str(line) + ".csv"
    )
