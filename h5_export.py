...
parser.add_argument(
    "outformat",
    choices=["ascii", "binary", "reflex"],
    help="Select which format to export to - either ascii, binary or reflex",
)
parser.add_argument("infile", help="input HDF (.h5) filename, with or without path")
parser.add_argument(
    "-o",
    "--outfile",
    help="output filename, basename only NO extension; defaults to infile",
)
parser.add_argument("-l", "--line", help="line number to export - defaults to all")
parser.add_argument(
    "--clobber", help="overwrite existing files", default=False, action="store_true"
)
